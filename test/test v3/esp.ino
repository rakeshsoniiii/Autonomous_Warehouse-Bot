...
void handleSetStatus(){
  ...
  if(server.hasArg("val")){
    ...
    botStatus = server.arg("val");

    Serial.println("STATUS SET: " + botStatus);
  }
...
void handleArucoMove(){

  String dir  = server.hasArg("dir")  ? server.arg("dir")          : "stop";
  int    ms   = server.hasArg("ms")   ? server.arg("ms").toInt()   : 0;

  if(dir == "left"){

    showLCD("ARUCO", "GO LEFT");
    setMotors(-180, 180);

  } else if(dir == "right"){

    showLCD("ARUCO", "GO RIGHT");
    setMotors(180, -180);

  } else if(dir == "forward"){

    showLCD("ARUCO", "FORWARD");
    setMotors(180, 180);

  } else if(dir == "back"){

    showLCD("ARUCO", "REVERSE");
    setMotors(-180, -180);
...
void setup() {
  ...
  server.on("/aruco_move", handleArucoMove);

  server.on("/status", handleStatus);

  server.on("/setstatus", handleSetStatus);
