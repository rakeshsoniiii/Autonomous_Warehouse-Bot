...
# Kitni door rukna hai (18cm center-to-center = Approx 10cm physical gap)
...
# Naya function Buzzer bajane ke liye
def send_beep_command():
    try:
        requests.get(f"{BASE_URL}/beep", timeout=0.5)
    except Exception:
        pass
    ...
    if state != "IDLE":
        ...
        if active_target in nodes_map and BOT_ID in nodes_map and px_per_cm is not None:
            ...
            if now - last_command_time > command_cooldown:
                # Updated Threshold to STOP_DISTANCE_CM (18cm)
                if dist_cm > STOP_DISTANCE_CM: # Driving Logic
                    ...
                    # Target Reached Logic (10cm Gap achieved)
                    if state == "GOING_TO_TARGET":
                        send_motor_command(0, 0)
                        send_beep_command() # Beep on object reach
                        state = "RETURNING_HOME"
                        print(f"Object Reached! Returning directly to Home (ID {HOME_ID})")
                    
                    elif state == "RETURNING_HOME":
                        send_motor_command(0, 0)
                        send_beep_command() # Beep on home reach
                        send_stop_mission() 
                        state = "IDLE"
