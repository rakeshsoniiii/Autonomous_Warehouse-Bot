...
current_target_id = 0
state = "IDLE" # IDLE, GOING_TO_2, WAITING_AT_2, GOING_TO_3
wait_start_time = 0
...
while True:
    ...
    try:
        if state == "IDLE":
            ...
            if res.get("mission") == 1 and res.get("target") == 2:
                state = "GOING_TO_2"
                print("Mission Started: Going to ID 2 at FULL SPEED")
    except Exception:
        pass
    ...
    if state != "IDLE":
        active_target = 2 if state == "GOING_TO_2" else (3 if state == "GOING_TO_3" else None)
        
        if state == "WAITING_AT_2":
            action_label = "WAITING 2 SEC AT ID 2..."
