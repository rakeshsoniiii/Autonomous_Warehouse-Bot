...
current_initial_id = 0
state = "IDLE" # IDLE, GOING_TO_INITIAL, WAITING_AT_INITIAL, GOING_TO_FINAL
wait_start_time = 0
...
while True:
    ...
    try:
        if state == "IDLE":
            ...
            if res.get("mission") == 1:
                ...
                if target in [2, 4, 5]: # Accept ID 2, 4, or 5 as starting point
                    current_initial_id = target
                    state = "GOING_TO_INITIAL"
                    print(f"Mission Started: Going to ID {current_initial_id} at FULL SPEED")
    except Exception:
        pass
    ...
    if ids is not None:
        for i, m_id in enumerate(ids.flatten()):
            if m_id in [BOT_ID, 2, 3, 4, 5]: # Add 4 and 5 to tracking scope
    ...
    if state != "IDLE":
        # Decide which target we are actively chasing
        active_target = current_initial_id if state == "GOING_TO_INITIAL" else (3 if state == "GOING_TO_FINAL" else None)
        
        if state == "WAITING_AT_INITIAL":
            action_label = f"WAITING 2 SEC AT ID {current_initial_id}..."
            send_motor_command(0, 0)
            if time.time() - wait_start_time > 2.0: # Exact 2 seconds wait
                state = "GOING_TO_FINAL"
                print("Switching Target: Going to ID 3 at FULL SPEED")
                
        elif active_target in nodes_map and BOT_ID in nodes_map and px_per_cm is not None:
            ...
            if now - last_command_time > command_cooldown:
                if dist_cm > 15.0: # Stop distance threshold
                    if abs(heading_error_deg) > 25: # Needs sharp alignment
                        if heading_error_deg > 0:
                            send_motor_command(255, -255) # Max Spin Right
                            action_label = f"SPINNING RIGHT TO ID {active_target}"
                        else:
                            send_motor_command(-255, 255) # Max Spin Left
                            action_label = f"SPINNING LEFT TO ID {active_target}"
                    else:
                        # Full speed forward with slight dynamic curve adjustment
                        ...
                        action_label = f"FULL SPEED TO ID {active_target}"
                else:
                    if state == "GOING_TO_INITIAL":
                        send_motor_command(0, 0)
                        state = "WAITING_AT_INITIAL"
                        wait_start_time = time.time()
                    elif state == "GOING_TO_FINAL":
                        send_motor_command(0, 0)
                        send_stop_mission() # Reset ESP32 state
                        state = "IDLE"
                        print("Mission Accomplished! Reached ID 3.")
    ...
    # UI Feedback
    cv2.putText(frame, f"STATUS: {state}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    cv2.putText(frame, f"ACT: {action_label}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
